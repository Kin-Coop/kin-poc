...
function civicrm_api3_job_recurringgenerate($params) {
  ...
  $new_payment_status_id = empty($contributionrecur_settings['complete']) ? 2 : 1;
  ...
  $select = 'SELECT cr.*, count(c.id) AS installments_done, NOW() as test_now
      ...
      WHERE
        ...
        AND (cr.payment_status_id IN (1,5))
  ';
  $spec = array();
  ...
  $update = 'UPDATE civicrm_contribution_recur cr
      INNER JOIN civicrm_payment_processor pp ON cr.payment_processor_id = pp.id
      SET
        cr.payment_status_id = 5
      WHERE
        cr.payment_status_id = 1
  ...
  $select = 'SELECT cr.*, pp.class_name, pp.is_test
      ...
      INNER JOIN civicrm_payment_processor pp ON cr.payment_processor_id = pp.id
      WHERE
        cr.payment_status_id = 5'.$param_where;
  //      AND pp.is_test = 0
  ...
  while ($dao->fetch()) {
    ...
    $contribution = array(
      ...
      'invoice_id'       => $hash,
      'source'         => $source,
      'payment_status_id' => $new_payment_status_id,
  ...
  $select = 'SELECT cr.id, count(c.id) AS installments_done, cr.installments
      ...
      WHERE
        (cr.installments > 0)
        AND (cr.payment_status_id  = 5) '.$param_where.'
  ...
  while ($dao->fetch()) {
    ...
    if ($dao->installments_done >= $dao->installments) { // I'm done with installments
      ...
      $update = 'UPDATE civicrm_contribution_recur SET payment_status_id = 1, end_date = NOW() WHERE id = %1';
      CRM_Core_DAO::executeQuery($update,array(1 => array($dao->id,'Int')));
    }
  }
