...
use Civi\ConfigItems\Entity\SimpleEntity\Exporter;
...
class ExportForm implements ConfigurationForm, ConfigurationFormCountable, Tab {
  ...
  public function buildConfigurationForm(\CRM_Core_Form $form, $configuration, $config_item_set) {
    ...
    $whereClauses = [];
    if ($entityDefinition->getAdditionalWhereClauses()) {
      $whereClauses = array_merge($whereClauses, $entityDefinition->getAdditionalWhereClauses());
    }
    ...
    try {
      if ($entityDefinition->getImporterClass()->entityImportDataExists($config_item_set)) {
  ...
  public function getConfigurationTemplateFileName() {
    return "CRM/ConfigItems/Entity/SimpleEntity/ExportForm.tpl";
  }
